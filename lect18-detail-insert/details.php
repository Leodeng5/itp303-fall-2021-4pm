...
// Passed from search_results.php
var_dump($_GET);
...
if(!isset($_GET["track_id"]) || empty($_GET["track_id"])) {
	...
	// It's nice to double check that my sql query looks good
	echo "<hr>" . $sql . "<hr>";
	...
	// For this query, we should be getting only ONE result. So no need for a while loop. The first result is all we need.
	$row = $results->fetch_assoc();

	// dump it out to see what kind of results we got back from the DB
	var_dump($row);
